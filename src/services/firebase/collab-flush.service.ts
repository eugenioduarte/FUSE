  ...
  for (const item of items) {
    try {
      ...
      if (item.url.includes('/sync/challenge')) {
        ...
        if (!isGroup) continue
        const { upsertGroupChallenge } = await import('./collab-data.service')
        ...
        if (!isGroup) continue
        const { upsertGroupSummary } = await import('./collab-data.service')
        ...
        if (!isGroup) continue
        const { upsertGroupTopic } = await import('./collab-data.service')
